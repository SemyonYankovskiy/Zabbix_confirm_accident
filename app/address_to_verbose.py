...
def get_info_from_zabbix_node(name: str) -> Tuple[str, str]:
    ...
    match = re.search(
        r"(?:SVSL|FTTB|MSAN)[-_](?:\d+[-_])?(\d{0,2}[a-z_A-Z]+)(\d+[abvwgdeABVWGDE.\d]{0,2})?(k\d{1,2})?\S*", name, flags=re.IGNORECASE)
    if match:
        ...
        #print(street_zabbix, house_zabbix, korpus_zabbix)

        if house_zabbix == None:
            house_zabbix = ""
